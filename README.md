...
Auth: `BYSTEP_TOKEN` env (a `bs_…` token from Settings → API keys) or the login saved by `npx @bystep/cli login`.
Defaults: `BYSTEP_PLAN_ID` / `BYSTEP_WORKSPACE_ID`, or `.bystep/config.json` written by `npx @bystep/cli connect`.
